...
class ReferenceService:
    ...
    def resolve(self, ref_id: str) -> ResolutionResult:
        ...
        2. File ID match (if indexed file moved but ID tracked - assumes ID persistence which we don't fully have yet in M1 without inode tracking, but indexer cleans up missing files).
           Wait, M1 indexer deletes missing files. So `file_id` is lost if file moves!
           Unless we implement move detection in M1 (we didn't).
           So `file_id` match only works if file is stable.
        ...
        if recorded_hash:
            ...
            matches = self.db.execute(
                "SELECT canonical_path, file_id FROM files WHERE content_hash = ?", 
                (recorded_hash,)
            ).fetchall()
